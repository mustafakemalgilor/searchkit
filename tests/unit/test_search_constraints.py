...
class TestLogFileDateSinceSeeker(TestSearchKitBase):
    ...
    def test_find_token_reverse(self):
        ...
        result = uut.find_token_reverse(100, 256)
    ...
    def test_find_token_reverse_eof(self):
        ...
        result = uut.find_token_reverse(77, 256)
    ...
    def test_find_token_eof(self):
        ...
        # Expectation: there is one LF between [0,100], status should be FOUND
        # and offset should be `78`
        result = uut.find_token(6896, 256)
        self.assertEqual(result.status, FindTokenStatus.REACHED_EOF)
    ...
    def test_find_token_fail(self):
        ...
        # Expectation: there is one LF between [0,100], status should be FOUND
        # and offset should be `78`
        result = uut.find_token(100000, 256)
        self.assertEqual(result.status, FindTokenStatus.FAILED)
    ...
    def test_try_find_line_elf_is_eof(self):
        ...
        self.assertEqual(result.start_lf.offset, 6837)
        self.assertEqual(result.end_lf.status, FindTokenStatus.REACHED_EOF)
        self.assertEqual(result.end_lf.offset, 6907)
        self.assertEqual(result.start_offset, 6838)
        self.assertEqual(result.end_offset, 6907)
        self.assertEqual(result.text,
    ...
    def test_try_find_line_verify_offsets(self):
        ...
        expected_offsets = set(
            [
                (0, 77), (79, 192), (194, 303), (305, 416),
                (418, 511), (513, 607), (609, 797), (799, 908),
                (910, 981), (983, 1101), (1103, 1220), (1222, 1351),
                (1353, 1433), (1435, 1557), (1559, 3125), (3127, 3272),
                (3274, 3383), (3385, 3457), (3459, 3522), (3524, 3633),
                (3635, 3714), (3716, 3807), (3809, 4050), (4052, 4218),
                (4220, 4279), (4281, 4388), (4390, 4485), (4487, 4543),
                (4545, 4651), (4653, 4787), (4789, 4917), (4919, 5014),
                (5016, 5086), (5088, 5175), (5177, 5206), (5208, 5213),
                (5215, 5225), (5227, 5237), (5239, 5240), (5242, 5249),
                (5251, 5252), (5254, 5259), (5261, 5262), (5264, 5276),
                (5278, 5279), (5281, 5281), (5283, 5371), (5373, 5432),
                (5434, 5508), (5510, 5623), (5625, 5707), (5709, 5820),
                (5822, 5961), (5963, 6038), (6040, 6185), (6187, 6326),
                (6328, 6386), (6388, 6403), (6405, 6426), (6428, 6443),
                (6445, 6478), (6480, 6495), (6497, 6513), (6515, 6550),
                (6552, 6601), (6603, 6679), (6681, 6759), (6761, 6836),
                (6838, 6907)
            ]
        )
    ...
    def test_try_find_line_with_date(self):
        ...
        expected_offsets = set(
            [
                ...
                (4220, 4279), (4281, 4388), (4390, 4485), (4487, 4543),
                (4545, 4651), (4653, 4787), (4789, 4917), (4919, 5014),
                (5016, 5086), (5088, 5175), (5283, 5371), (5373, 5432),
                (5434, 5508), (5510, 5623), (5625, 5707), (5709, 5820),
                (5822, 5961), (5963, 6038), (6040, 6185), (6187, 6326),
                (6328, 6386), (6603, 6679), (6681, 6759), (6761, 6836),
                (6838, 6907)
            ]
        )
        for i in range(0, len(MYSQL_LOGS)):
